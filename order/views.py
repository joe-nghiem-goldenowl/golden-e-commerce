...
def add_to_cart(request):
    ...
    try:
        order, _ = Order.objects.get_or_create(customer_id=request.user.id, is_paid=False)

        order.producttype_set.create(product_id=product_id, quantity=product_quantity, size=product_size, color=product_color)
